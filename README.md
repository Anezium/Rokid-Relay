...
On first setup, install only the phone APK manually: after Hi Rokid authorization and glasses connection, the phone app automatically uploads, installs, and starts this bundled glasses helper through CXR-L. Later phone updates re-upload the bundled helper when its APK fingerprint changes.
...
Usually the phone app deploys and starts the glasses helper automatically after Hi Rokid authorization, so a normal first install only needs the phone APK. For direct glasses debugging, install the glasses APK manually:
