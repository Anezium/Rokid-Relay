...
On first setup, install only the phone APK manually: after Hi Rokid authorization and glasses connection, the phone app automatically uploads, installs, and starts this bundled glasses helper through CXR-L while the phone app is open. Background autostarts can prepare or update the helper, but they do not bring the glasses helper activity to the foreground. Later phone updates re-upload the bundled helper when its APK fingerprint changes.
...
Building the phone APK also builds and copies the glasses APK into the phone app assets so the phone can install the helper through Hi Rokid when the CXR-L path is ready.
