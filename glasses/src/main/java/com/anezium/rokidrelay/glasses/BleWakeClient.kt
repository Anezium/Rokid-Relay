...
object BleWakeClient {
    ...
    @Volatile private var activeCallback: ((Boolean, String) -> Unit)? = null
    ...
    @Volatile private var activeNotificationId: String = ""
    ...
    fun requestWake(
        ...
        main.post {
            ...
            if (notificationId.isBlank()) {
                finish(false, "No replyable notification")
                return@post
            }
            if (!hasBlePermissions(appContext)) {
                finish(false, "Bluetooth permission missing. Open Relay once.")
                return@post
            }
    ...
    private fun stopScan(context: Context) {
        ...
        runCatching {
            ...
            Log.w(TAG, "stop scan failed: ${it.message}")
        }
    }
    ...
    private val gattCallback = object : BluetoothGattCallback() {
        ...
        override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {
            ...
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                ...
                finish(false, "BLE wake disconnected")
            }
        }
    ...
    private fun writeWakeRequest(
        ...
        val payload = JSONObject()
            ...
            .put("type", "wake_reply")
            .put("source", "glasses")
            .put("notificationId", activeNotificationId)
            .toString()
    ...
    private fun cancelActive(notify: Boolean) {
        ...
        if (context != null && scanCallback != null) {
            runCatching {
                ...
                Log.w(TAG, "cancel scan failed: ${it.message}")
            }
        }
        activeScanCallback = null
        activeContext = null
        runCatching { activeGatt?.close() }.onFailure {
            Log.w(TAG, "close wake GATT failed: ${it.message}")
        }
        activeGatt = null
